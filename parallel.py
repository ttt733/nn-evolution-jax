...
flags.DEFINE_integer('npop_per_device', 64,
                     'Population size to run on each device for optimization')
...
flags.DEFINE_integer('noise_population_batches', 4,
                     "Number of batches to divide the population's noise into")
flags.DEFINE_integer('noise_split_batches', 3,
                     'Number of batches to divide parameter arrays into')
FLAGS = flags.FLAGS
...
def main(_):
    ...
    initial_weight_decay = jnp.sum(jnp.multiply(flat_params, flat_params))

    def get_weight_decay(params):
        return jnp.abs(initial_weight_decay) - jnp.sum(jnp.multiply(params, params))
    ...
    def test_infer(rng, flat_params):
        ...
        reward = reward_fn(fwd_fn_apply, test_params, rng,
                           obs, targets, get_weight_decay(flat_params))
    ...
    npop = FLAGS.npop_per_device * ndevices
    epochs = FLAGS.epochs
    ...
    noiseremainder = nnoisesplits - total_params % nnoisesplits
    ...
    def run_member(rng, base_params, obs, targets):
        ...
        reward = reward_fn(fwd_fn_apply, unravel_params(
            noised_params), rng, obs, targets, get_weight_decay(noised_params))
    ...
    def run_epoch(carry, rng):
        ...
        population_rng = einops.rearrange(
            population_rng, '(a b) c -> a b c', a=ndevices)
        rng_out, reward_out = p(population_rng, flat_params, obs, targets)

        # Combine incoming values from devices
        rng_out = einops.rearrange(rng_out, 'a b c -> (a b) c')
        reward_out = einops.rearrange(reward_out, 'a b c -> (a b) c')

        # Batch results before noise calculation
        rng_out = einops.rearrange(
            rng_out, '(a b) c -> a b c', a=nnoisebatches)
        reward_out = einops.rearrange(
            reward_out, '(a b) c -> a b c', a=nnoisebatches)
